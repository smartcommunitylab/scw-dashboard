...
export class DbParticipationComponent implements OnInit {
  ...
  private updateData(event?: number) {
    ...
    //call functions that fetch data from ElasticSearch
    ...
    this.updateMonthData(this.currentCity, moment(day).toDate().getTime(), this.currentTime);
    /*
  ...
  private updateMonthData(city: string, from: number, to: number) {
    //TODO
    let daySumQuery = {query: {bool: {must : [ {match : {eventType : 'UserCreation'}}, {match : {residence : city}}, { range : { executionTime : {from : from, to : to}}}]}}};
    this.http.post(API + '_count', daySumQuery).subscribe((res: any) => {
